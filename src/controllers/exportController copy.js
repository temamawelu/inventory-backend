...
class ExportController {
    async exportProductsToExcel(req, res) {
        ...
        try {
            const [products] = await pool.query('SELECT p.id, p.sku, p.name, p.description, c.name as category, p.unit_price, p.quantity_on_hand, p.reorder_point, p.location FROM products p  LEFT JOIN categories c ON p.category_id = c.id  WHERE p.is_active = 1  ORDER BY p.name ASC');
            ...
            worksheet.addRow({});
            worksheet.addRow({ sku: 'TOTAL', quantity_on_hand: totalStock, unit_price: totalValue });
    ...
    async exportTransactionsToExcel(req, res) {
        ...
        try {
            const [transactions] = await pool.query('SELECT it.id, p.name as product_name, it.transaction_type, it.quantity, it.quantity_before, it.quantity_after,it.reference_number, it.notes, it.created_at,u.username as user_name FROM inventory_transactions it JOIN products p ON it.product_id = p.id JOIN users u ON it.user_id = u.id ORDER BY it.created_at DESC');
    ...
    async exportLowStockToExcel(req, res) {
        ...
        try {
            const [lowStock] = await pool.query('SELECT p.sku, p.name, p.quantity_on_hand, p.reorder_point,  p.location, c.name as category FROM products p LEFT JOIN categories c ON p.category_id = c.id WHERE p.is_active = 1 AND p.quantity_on_hand <= p.reorder_point ORDER BY p.quantity_on_hand ASC');
